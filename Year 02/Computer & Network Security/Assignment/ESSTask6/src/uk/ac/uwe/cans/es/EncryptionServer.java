...
public class EncryptionServer {
    ...
    private ServerSocket sslSocket = null;

    public EncryptionServer() {
        System.out.println("Encryption Server initialised.");
        // Open server socket for listening
        try {
            sslSocket = new ServerSocket(SERVER_SSL_LISTENING_PORT);
            System.out.println("EncryptionServer listening on port " + SERVER_SSL_LISTENING_PORT);
        } catch (IOException se) {
            System.err.println("EncryptionServer cannot listen on port " + SERVER_SSL_LISTENING_PORT);
            System.exit(-1);
        }
